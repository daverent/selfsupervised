...
def use_SimSiam(configs):
    ...
    model_path = "./SimSiam/Models/SimSiam_"+str(configs['path_to_train'].split('/')[-2])+"_"+str(configs['max_epochs'])+"ep.pt" #Standard torch.save() doesn't work, saving state_dict
    ...
    if os.path.exists(classifier_path):
        classifier = SAClassifier(backbone=model.online_network.encoder, num_classes=num_classes)
    ...
    if want_confmat:
        ...
        with open('./Reports/SimSiam_'+ str(configs['max_epochs']) +'ep_ClassificationReport.txt', mode='w') as savefile:
